...
func TestPutWithTTL_UpdateExisting(t *testing.T) {
	c := NewResponseCache(Config{MaxSize: 100})
	c.PutWithTTL("k", "m", "p", "r1", 1, 0.001, time.Hour)
	c.PutWithTTL("k", "m", "p", "r2", 1, 0.002, time.Hour)
	got := c.Get("k")
	if got == nil {
		t.Fatal("expected to find entry")
	}
	if got.Response != "r2" {
		t.Errorf("expected updated response r2, got %s", got.Response)
	}
}

func TestPut_UpdateExisting(t *testing.T) {
	c := NewResponseCache(Config{MaxSize: 100})
	c.Put("k", "m", "p", "r1", 1, 0.001, nil)
	c.Put("k", "m", "p", "r2", 1, 0.002, nil)
	got := c.Get("k")
	if got == nil {
		t.Fatal("expected to find entry")
	}
...
func TestEvictOldest_EmptyOrder(t *testing.T) {
	c := NewResponseCache(Config{MaxSize: 100})
	// Directly clear order to simulate inconsistency
	c.mu.Lock()
	c.order = nil
	c.mu.Unlock()
	// Should not panic
